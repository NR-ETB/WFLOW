...
const previewPath = path.join(previewDir, 'v11.9-responsive-homogeneo.html');
const workflow = JSON.parse(fs.readFileSync(workflowPath, 'utf8'));
fs.mkdirSync(previewDir, { recursive: true });
const formsDir = path.join(previewDir, 'v11.9-forms');
...
for (const node of forms) {
  ...
  generated.push({ name: node.name, file: `v11.9-forms/${slug}.html` });
