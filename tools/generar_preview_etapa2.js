...
const workflow = JSON.parse(fs.readFileSync(path.join(root, 'Ningun Servicio Funciona - 2.json'), 'utf8'));
const outputDir = path.join(root, 'preview', 'etapa2-v1');
...
for (const node of forms) {
  ...
  const result = render(
    { id: 'preview-etapa2', mode: 'test', resumeUrl: 'https://n8n.example.test/webhook-waiting/preview-etapa2' },
    { query: { workflow_session: 'preview-session', tipo_sim: 'eSIM', linea_portada: 'Si' } },
  );
...
console.log(`Previews Etapa 2: ${manifest.length}`);
